...
const Comments = ({blog}) => {
    ...
    const update = () => {
        fetch("/api/blog/comments/?blog=" + blog.id).then(async (r) => {
    ...
    const send = () => {
        fetch("/api/blog/comments/?blog=" + blog.id, {
            method: "POST",
            headers: {
    ...
    return (
        <Col md={6} className="bg-light rounded mx-md-auto m-2 p-2">
            ...
            {comments.length > 0 ? comments.map(x => (
                <div className="bg-primary bg-opacity-25 rounded mb-1 p-2">
                    ...
                    {x.reply ? <Alert className="m-1 p-2" variant="warning">پاسخ: {x.reply}</Alert> : null}
                </div>
